...
4. 或者打开 **账号注册** Tab → 选择本次打码服务，再配置对应验证码参数 + 默认邮箱渠道所需配置（如 `GPTMAIL_API_KEY` / `YYDSMAIL_API_KEY` / `MOEMAIL_API_KEY`）→ 点击手动注册
5. `yescaptcha` 需要配置 `YESCAPTCHA_KEY`；`ohmycaptcha_local` 默认使用 `http://127.0.0.1:8001` 与默认 key `ohmycaptcha-local-key`
6. 如需使用 YesCaptcha，可访问 [YESCAPTCHA](https://yescaptcha.com/i/w9X0Ae) 获取 API Key
